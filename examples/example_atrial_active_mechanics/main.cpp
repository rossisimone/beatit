...
int main(int argc, char ** argv)
{
	...
	std::string datafile_name = commandLine.follow("em.beat", 2, "-i",
			"--input");
	...
	if (do_restart)
	{
		std::string restart_file = data("monodomain/restart/restart_file",
				"NONE");
	...
	if (do_restart == false)
	{
		...
		else
		{
			...
			std::cout
					<< "Either you did not specify the mesh in mesh/input_mesh_name"
					<< std::endl;
			std::cout
					<< "Or you want to do a simulation of Pippo,  which (I agree) it would be nice."
					<< std::endl;
	...
	if (do_restart)
	{
		...
		std::string fibers_file = data("monodomain/restart/fibers_file",
				"NONE");
...
//        }
	...
	if (buildFibers)
	{
		...
		auto& grad2 = poisson2.M_equationSystems.get_system
				< libMesh::ExplicitSystem > (pois2 + "_gradient").solution;
		auto& grad3 = poisson3.M_equationSystems.get_system
				< libMesh::ExplicitSystem > (pois3 + "_gradient").solution;
		auto& grad4 = poisson4.M_equationSystems.get_system
				< libMesh::ExplicitSystem > (pois4 + "_gradient").solution;
		...
		poisson2.M_equationSystems.get_system < libMesh::LinearImplicitSystem
				> (pois2).update();
		std::cout << "\nGetting pois 2 solution:  ... " << std::flush;
		auto& sol_pois2 = poisson2.M_equationSystems.get_system
				< libMesh::LinearImplicitSystem
				> (pois2).current_local_solution;
		auto& sol_pois1 = poisson2.M_equationSystems.get_system
				< libMesh::LinearImplicitSystem
				> (pois1).current_local_solution;
		std::cout << "\nGetting dof map:  ... " << std::flush;
		const libMesh::DofMap & dof_map = poisson1.M_equationSystems.get_system
				< libMesh::ExplicitSystem > (pois1 + "_gradient").get_dof_map();
		std::cout << "\nGetting dof map phi:  ... " << std::flush;
		const libMesh::DofMap & dof_map_phi =
				poisson2.M_equationSystems.get_system
						< libMesh::LinearImplicitSystem > (pois2).get_dof_map();
		std::cout << " done \n " << std::flush;
		...
		for (; el != end_el; ++el)
		{
			...
			double phi = 0;
			double phi1 = 0;
			for (auto && index : dof_indices_phi)
			{
				phi += (*sol_pois2)(index) / nnodes;
				phi1 += (*sol_pois1)(index) / nnodes;
			}
			...
			if (blockID == 3)
			{
				...
				if (phi >= layer_threshold)
				{
					...
					grad4->set(dof_indices[0], xfx);
					grad4->set(dof_indices[1], xfy);
					grad4->set(dof_indices[2], xfz);
				}
				else
				{
					...
					grad4->set(dof_indices[0], fx);
					grad4->set(dof_indices[1], fy);
					grad4->set(dof_indices[2], fz);

				}
			}
			else if (blockID == 1 || blockID == 2 || blockID == 4
					|| blockID == 19 || phi1 < 0.15)
			{
				...
				{
					grad4->set(dof_indices[0], cross_x);
					grad4->set(dof_indices[1], cross_y);
					grad4->set(dof_indices[2], cross_z);
					grad3->set(dof_indices[0], v1_x);
					grad3->set(dof_indices[1], v1_y);
					grad3->set(dof_indices[2], v1_z);
		...
		auto& fibers = es.get_system < libMesh::ExplicitSystem
				> ("fibers").solution;
		auto& sheets = es.get_system < libMesh::ExplicitSystem
				> ("sheets").solution;
		auto& xfibers = es.get_system < libMesh::ExplicitSystem
				> ("xfibers").solution;
