...
//    {
    ...
    auto& grad4 = poisson3. M_equationSystems.get_system<libMesh::ExplicitSystem>(pois4+"_gradient").solution;
...
//  const libMesh::DofMap & dof_map = poisson1.M_equationSystems.get_system<libMesh::ExplicitSystem>("gradient").get_dof_map();
       std::vector<libMesh::dof_id_type> dof_indices;
    	 ...
    	 const auto blockID = elem->subdomain_id();
          ...
          double xfx = (*grad4)(dof_indices[0]);
    	  double xfy = (*grad4)(dof_indices[1]);
    	  double xfz = (*grad4)(dof_indices[2]);
...
//    auto& grad3 = poisson3. M_equationSystems.get_system<libMesh::ExplicitSystem>(pois3+"_gradient").solution;
    ...
    {
        BeatIt::Monowave monowave(es);
        monowave.setup(data, "monodomain");
        monowave.init(0.0);
        unsigned int  bID = data("monodomain/bID", 333);

        monowave.set_potential_on_boundary(bID);

        // Fibers
        auto& fibers = monowave.M_equationSystems.get_system<libMesh::ExplicitSystem>("fibers").solution;
        // Sheets
        auto& sheets = monowave.M_equationSystems.get_system<libMesh::ExplicitSystem>("sheets").solution;
        // XFibers
        auto& xfibers = monowave.M_equationSystems.get_system<libMesh::ExplicitSystem>("xfibers").solution;
        ...
        *xfibers = *grad3;
        ...
        double  endo_angle = data("monodomain/endo_angle", 0.0);
        double  epi_angle = data("monodomain/epi_angle", 0.0);
        ...
        monowave.form_system_matrix(datatime.M_dt, false);
        std::cout << "Done" << std::endl;
        ...
        for( ; datatime.M_iter < datatime.M_maxIter && datatime.M_time < datatime.M_endTime ; )
        {
            ...
            monowave.update_activation_time(datatime.M_time);
