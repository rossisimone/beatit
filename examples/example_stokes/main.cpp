...
//	equation_systems.parameters.set < Real > ("linear solver tolerance") = TOLERANCE;
    ...
    ExodusII_IO(mesh).write_equation_systems ("out_hh16.e",
            equation_systems);
...
void assemble_stokes(EquationSystems &es, const std::string& libmesh_dbg_var(system_name))
{
	...
	DenseSubMatrix<Number> Kuu(Ke), Kuv(Ke), Kuw(Ke), Kup(Ke), Kvu(Ke), Kvv(Ke), Kvw(Ke), Kvp(Ke), Kwu(Ke), Kwv(Ke), Kww(Ke), Kwp(Ke), Kpu(Ke), Kpv(Ke), Kpw(
			Ke), Kpp(Ke);
...
//                    std::unique_ptr<const Elem> side_elem_ptr(elem->build_side_ptr(s));
				...
				if ( boundary_id >= 5 )
				{
					...
					for (unsigned int qp = 0; qp < qface.n_points(); qp++)
					{
						...
						double inflow_magnitude = 1.0 * phi_value;
						double ux = inflow_magnitude * normals[qp](0);
						double uy = inflow_magnitude * normals[qp](1);
						double uz = inflow_magnitude * normals[qp](2);
						double penalty = 1e8;
						// Assemble RHS BC
						for (unsigned int l = 0; l < phi_face.size(); l++)
						{
