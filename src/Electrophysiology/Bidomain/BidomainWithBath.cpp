...
void BidomainWithBath::setupSystems(GetPot& data, std::string section)
{
    ...
    std::cout
            << "* BIDOMAIN+BATH: Creating new System for the hyperbolic bidomain equations"
            << std::endl;
...
//  bidomain_system.reinit_constraints();
    ...
    // ///////////////////////////////////////////////////////////////////////
    // 2) ODEs
    std::cout << "* BIDOMAIN+BATH: Creating new System for the ionic model "
            << std::endl;
...
void BidomainWithBath::initSystems(double time)
{
    ...
    if (v_ic != "")
    {
        std::cout << "* BIDOMAIN+BATH: Found bidomain initial condition: " << v_ic
                << std::endl;
        ...
        std::cout
                << "* BIDOMAIN+BATH: Projecting initial condition to bidomain system ... "
                << std::flush;
...
//    wave_system.solution->print();
    std::cout
            << "* BIDOMAIN+BATH: Copying initial conditions to vectors at n nd at n-1... "
            << std::flush;
...
//    bidomain_system.solution->print();
    ...
    IonicModelSystem& istim_system = M_equationSystems.get_system
            < IonicModelSystem > ("istim");
    std::cout << "* BIDOMAIN+BATH: Initializing activation times to -1  ... "
            << std::flush;
    ...
    std::cout << "* BIDOMAIN+BATH: Creating fibers from function: " << fibers_data
            << std::flush;
    fiber_system.project_solution(&fibers_func);
    std::cout << " done" << std::endl;
    std::cout << "* BIDOMAIN+BATH: Creating sheets from function: " << sheets_data
            << std::flush;
    sheets_system.project_solution(&sheets_func);
    std::cout << " done" << std::endl;
    std::cout << "* BIDOMAIN+BATH: Creating xfibers from function: " << xfibers_data
            << std::flush;
...
void BidomainWithBath::form_system_rhs(double dt, bool useMidpoint,
        const std::string& mass)
{
...
void BidomainWithBath::solve_diffusion_step(double dt, double time, bool useMidpoint,
