...
void Grandi11::initializeSaveData(std::ostream& output)
{
	// time -  0
	output << "time v ";
...
void Grandi11::initialize(std::vector<double>& variables)
{
...
void Grandi11::setup(GetPot& data, std::string sect)
{
	std::string section = sect + "/Grandi11";
 ...
 */
void Grandi11::solve(std::vector<double>& variables, double appliedCurrent,
		double dt)
{
 ...
 *  \param [in] dt        Timestep
 */
double Grandi11::evaluateIonicCurrent(std::vector<double>& variables,
...
//Nernst Potentials
void Grandi11::nernst_potentials(const std::vector<double>& variables)
{
	//Najo=9.136;%8.80329;
...
double Grandi11::evaluatedIonicCurrent(std::vector<double>& variables,
		double /*appliedCurrent*/, double /*dt*/, double /*h*/)
{
	// Membrane Currents
	...
	double aux = 0.0;
	// INa
	double dI_Na_junc = Fjunc * GNa * m * m * m * h * j;
	double dI_Na_sl = Fsl * GNa * m * m * m * h * j;
	double dI_Na = dI_Na_junc + dI_Na_sl;

	double dI_NaL_junc = Fjunc * GNaL * Inal1 * Inal1 * Inal1 * Inal2;
	double dI_NaL_sl = Fsl * GNaL * Inal1 * Inal1 * Inal1 * Inal2;
	double dI_NaL = dI_NaL_junc + dI_NaL_sl;

	// I_nabk: Na Background Current
	double dI_nabk_junc = Fjunc * GNaB;
	double dI_nabk_sl = Fsl * GNaB;
	...
	double dI_kr = gkr * xkr * drkr * (v - ek) + gkr * xkr * rkr;
	...
	if (!markov_iks)
	{
		...
		double dI_ks_junc = Fjunc * gks_junc * xks * xks;
		double dI_ks_sl = Fsl * gks_sl * xks * xks;
	...
	else
	{
		...
		double O2 = 1
				- (C1 + C2 + C3 + C4 + C5 + C6 + C8 + C7 + C9 + C10 + C11 + C12
						+ C13 + C14 + C15 + O1);
		double dI_ks_junc = Fjunc * gks_junc * (O1 + O2);
		double dI_ks_sl = Fsl * gks_sl * (O1 + O2);
		dI_ks = dI_ks_junc + dI_ks_sl;
	}
	...
	double dI_kp = dI_kp_junc + dI_kp_sl;

	// I_to: Transient Outward K Current (slow and fast components)

	//ytof += dt *( ytoss - ytof )  / tauytof;
	double dI_tof = 1.0 * GtoFast * xtof * ytof;
	double dI_to = 1.0 * dI_tof;
	...
	double dI_kur = 1 * Gkur * rkur * skur;
	...
	double dI_ClCa_junc = Fjunc * GClCa / (1 + KdClCa / Caj);
	double dI_ClCa_sl = Fsl * GClCa / (1 + KdClCa / Casl);
	...
	double dI_ncx_junc = Fjunc * IbarNCX * aux * Ka_junc * (ds1_junc - ds2_junc)
	...
	double dI_ncx_sl = Fsl * IbarNCX * aux * Ka_sl * (ds1_sl - ds2_sl) / s3_sl
			/ (1 + ksat * std::exp((nu - 1) * v * FoRT))
			- Fsl * IbarNCX * aux * Ka_sl * (s1_sl - s2_sl) / s3_sl
					* (ksat * ((nu - 1) * FoRT) * std::exp((nu - 1) * v * FoRT))
					/ (1 + ksat * std::exp((nu - 1) * v * FoRT))
					/ (1 + ksat * std::exp((nu - 1) * v * FoRT));
	...
	double dI_pca_junc = 0.0;
	double dI_pca_sl = 0.0;
	double dI_pca = dI_pca_junc + dI_pca_sl;
	// I_cabk: Ca Background Current
	double dI_cabk_junc = Fjunc * GCaB;
	double dI_cabk_sl = Fsl * GCaB;
...
void Grandi11::updateVariables(std::vector<double>& variables,
		double appliedCurrent, double dt)
{
