...
double
Grandi11::evaluatedIonicCurrent(std::vector<double>& variables, double /*appliedCurrent*/, double /*dt*/, double /*h*/)
{
    // Membrane Currents
    ...
    aux = KmNaip / Naj;
    double dI_nak_junc = Fjunc * IbarNaK * dfnak * Ko / ( 1.0 + aux * aux * aux * aux ) / ( Ko + KmKo );
    aux = KmNaip / Nasl;
    double dI_nak_sl = Fsl * IbarNaK * dfnak * Ko / ( 1.0 + aux * aux * aux * aux ) / ( Ko + KmKo );
    ...
    double dI_ncx_junc = Fjunc*IbarNCX*aux*Ka_junc*(ds1_junc-ds2_junc)/s3_junc/(1.0+ksat*std::exp((nu-1)*v*FoRT))
    ...
    double dI_ncx_sl = Fsl*IbarNCX*aux*Ka_sl*(ds1_sl-ds2_sl)/s3_sl/(1+ksat*std::exp((nu-1)*v*FoRT))
